...
MTU_VALUE="1400"
SHADOWBOX_API_PORT="8081"  # Порт для Management API (панель управления)
SHADOWSOCKS_PORT="8388"    # Порт для Shadowsocks-трафика
...
check_containers_and_print_final() {
  ...
  # (ss -tuln или netstat -tuln, здесь приведём пример c ss)
  ...
  if [ -f "${OUTLINE_DIR}/access.txt" ]; then
    ...
    # Предположим, что публичный IP — это IP вашей машины (или хостнейм). Можно вывести команду curl для auto-detect
    # Для упрощения предположим, что пользователь сам знает свой IP. Или используем icanhazip:
    ...
    LOG_INFO "Если требуется, откройте порты ${API_PORT} (TCP) и ${SHADOWSOCKS_PORT} (TCP/UDP) в фаерволе/облаке."
  else
